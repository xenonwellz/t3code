...
// adapter keyed by its driver kind. To test the facade we supply five fake
...
const makeFakeInstance = (
  driverKindString: "codex" | "claudeAgent" | "cursor" | "kimi" | "opencode",
...
const fakeInstances: ReadonlyArray<ProviderInstance> = [
  ...
  makeFakeInstance("kimi", fakeKimiAdapter),
];
  ...
  it("resolves adapters and routing metadata from provider instances", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(instances, [
        ...
        defaultInstanceIdForDriver(KIMI_DRIVER),
      ]);
